...
- 应用场景
  ...
  - Windows的**Task Manager任务管理器**就是很典型的单例模式。
  - Windows的**Recycle Bin回收站**也是典型的单例应用。在整个系统运行过程中，回收站一直维护着仅有的一个实例。
...
7. 不可变性好处：
   1. 字符串常量池，性能优化，相同的字符串常量不用重新创建。
   2. 缓存hashcode，String类中会缓存hashcode，只需计算一次。
   3. 多线程安全，多线程共享，不需要做任何同步处理。
...
- 扩容的条件变为只要超过临界值就扩容
- 红黑树退化：
  - 满足链表>=8时调用treefy()如果小于64则会退化为resize
  - 扩容时，红黑树会根据最高位分为low树和high树，如果容量小于6退化为链表
...
<https://www.shangmayuan.com/a/84247b009e1a4776a3b3ce24.html>
