...
* 每个Java方法在执行的同时会创建一个栈帧用于存储**局部变量表、操作数栈、常量池引用**等信息。
    * 局部变量表存放了:
...
* 现代的垃圾收集器基本都是采用**分代收集算法**，其主要的思想是针对不同类型的对象采取不同的垃圾回收算法。
...
* JDK1.7以前通过永久代实现，使用**堆内存空间**。
* 在 JDK 1.8 之后，原来**永久代的数据被分到了堆和元空间中**。
    * 元空间：存储类的元信息，
    * 堆：静态变量和常量池。
        * 运行时常量池
...
* 在 JDK 1.4 中新引入了 NIO 类，它可以使用 Native 函数库直接分配堆外内存，然后通过 Java 堆里的 DirectByteBuffe 对象作为这块内存的引用进行操作。
...
2. 双亲委托：当前类加载器(以系统类加载器为例)在加载一个类时，委托给其双亲（**注意这里的双亲指的是类加载器中parent属性指向的类加载器**）先进行加载。
4. 如此反复
...
因为在某些情况下，父类加载器无法加载到需要的文件，**父类加载器需要委托子类加载器去加载class文件**。
...
* **引用计数算法**：
    ...
    * 缺点：难处理**循环引用关系**。
* **Java 选择的可达性分析**（Tracing Garbage Collection）
...
要真正宣告一个**对象死亡，至少要经历两次标记过程**：

1. **如果对象在进行可达性分析后发现，没有与GC Roots相连接的引用链，那它将会被第一次标记。**
2. 随后进行一次筛选，筛选的条件是此对象**是否有必要执行finalize()方法**。
    1. 假如对象没有覆盖finalize()方法，或者finalize()方法已经被虚拟机调用过，那么虚拟机将这两种情况都视为“没有必要执行”。
3. 如果这个对象被判定为**有必要执行finalize()方法**，那么该对象将会被放置在一个名为F-Queue的队列之中，并在稍后由一条由虚拟机自动建立的、低调度优先级的Finalizer线程去执行它们的finalize()方法。
    2. 这样做的原因是，如果某个对象的finalize()方法执行缓慢，或者更极端地发生了死循环，将很可能导致FQueue队列中的其他对象永久处于等待，甚至导致整个内存回收子系统的崩溃。
    2. **finalize()方法是对象逃脱死亡命运的最后一次机会，稍后收集器将对F-Queue中的对象进行第二次小规模的标记**，
        1. 如果对象要在finalize()中成功拯救自己——只要重新与引用链上的任何一个对象建立关联即可，譬如把自己（this关键字）赋值给某个类变量或者对象的成员变量，那在第二次标记时它将被移出“即将回收”的集合；
        2. 如果对象这时候还没有逃脱，那基本上它就真的要被回收了。
...
1. Minor GC 触发时机：**在新生代的Eden区域满了之后就会触发 gc，采用复制算法来回收新生代的垃圾。**
2. 对象在新生代的分配与回收
    1. 当 Eden 区将满时，触发 Minor GC
    2. 第一次Minor GC 时：
        1. 会把 Eden 区的存活对象和 S0中的存活对象一起移到 S1
        2. 同时清空 Eden 和 S0 的空间。
    3. 第二次Minor GC时：
        1. 从 Eden，S1 区将存活对象复制到 S0 区
        2. 每次垃圾回收, S0, S1 角色互换，都是从 Eden ,S0(或S1) 将存活对象移动到 S1(或S0)。
3. 对象何时晋升老年代？
...
* 这个时间点的选定既不能太少，以让 GC 时间太长导致程序过长时间卡顿，也不能过于频繁，以至于过分增大运行时的负荷。
...
**CMS（Concurrent Mark Sweep）收集器是一种以获取最短回收停顿时间为目标的收集器**。GC工作线程与用户线程可以并发执行，以此来达到降低收集停顿时间的目的。**CMS 收集器是以实现最短 STW 时间为目标的收集器**
...
1. **运作期间不会产生内存碎片**，
    1. **CMS：标记-清除算法**
    2. **G1 从整体上看采用的是标记-整理法，局部（两个 Region）上看是基于复制算法实现的，**
    3. 两个算法都不会产生内存碎片，收集后提供规整的可用内存，这样有利于程序的长时间运行。
...
* **有计划地避免在整个Java堆中进行全区域的垃圾收集**。
    ...
    * 收集器在给定了“停顿”时间限制的情况下，总是能选择一组恰当的Regions作为收集目标，让其收集开销满足这个限制条件，以此达到实时收集的目的。
...
1. **初始标记（Initial Marking）**：仅仅只是标记一下GC Roots能直接关联到的对象，并且修改TAMS（Next Top at Mark Start）的值，让下一阶段用户程序并发运行时，能在正确可用的Region中创建新对象，**这阶段需要`停顿线程`，但耗时很短**。
2. **并发标记（Concurrent Marking）**：是从GC Roots开始堆中对象进行可达性分析，找出存活的对象，**这阶段耗时较长**，但可与用户程序并发执行。
3. **最终标记（Final Marking）**：是为了修正并发标记期间因用户程序继续运作而导致标记产生变动的那一部分标记记录，虚拟机将这段时间对象变化记录在线程Remembered Set Logs里面，最终标记阶段需要把Remembered Set Logs的数据合并到Remembered Set中，**这阶段需要`停顿线程`，但是可并行执行**。
